...
# ==============================================================================
# Tree Operators (from HALO/operators.py)
# ==============================================================================
...
def prolongate_potentials(
    ...
    """
    [HALO 兼容] 将粗层势能 (u, v) 传递到细层 (k+1 -> k)。

    接口与 HALO/operators.py:prolongate_potentials_tree 一致:
    - 输入: (x_coarse, level_X_now, level_Y_now, level_X_coarse, level_Y_coarse)
    - 输出: X+Y 部分的势能

    子节点的势能 = 其父节点的势能。
    使用 parent_labels 进行映射。
    """
...
def _refine_expand_numba(
    ...
    """
    Numba 核心: 根据 coarse 索引 + children CSR，展开成细层变量。
    """
    Kc = idx_X_coarse.shape[0]
    
    # 1. 计算每个 coarse var 的 block size
    ...
    for i in prange(Kc):
        ...
        for a in range(m):
            ...
            for b in range(n):
                ...
                # HALO 对齐：每个子边继承 coarse y 值，不做均分。
                y_init_y[pos] = yv
...
def refine_duals(
    ...
    dtype=np.float32,
    thr: float = 1e-20,
    timing: bool = False,  # 添加 timing 参数
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    将粗尺度的 y 解传递到细层 (k+1 -> k)。

    参数:
    ----
    y_solution_coarse: 粗尺度的 y 解 {"y": ..., "keep": ...}
    level_fine_X: 细层 X (source) 等级
    level_fine_Y: 细层 Y (target) 等级
    level_coarse_X: 粗层 X (source) 等级
    level_coarse_Y: 粗层 Y (target) 等级
    """
    ...
    if timing:
        ...
        print(f"      [RefineDuals-hier] 结果: K_fine={len(y_init_keep_1d)}, time={t_end-t_start:.3f}s")
...
def generate_minus_c(
    ...
    """根据树结构计算 -C_{ij}（适用 L1, L2(=l2^2), Linf）"""
