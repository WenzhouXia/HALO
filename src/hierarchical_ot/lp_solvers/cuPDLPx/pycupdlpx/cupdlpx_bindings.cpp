...
public:
  ...
  // ====== 新增：直接接收 scipy.sparse.csc_matrix ======
...
static py::dict make_default_params()
{
  ...
  // === 新增：步长相关的默认 ===
  d["step_size_method"] = 0;     // 0=power,1=one_inf,2=hybrid（推荐默认 hybrid）
  d["step_size_safety"] = 0.998; // 或者 0.998，按你需要

  d["power_max_iterations"] = 20; // power/hybrid 使用
  d["power_tolerance"] = 1e-4;
  d["hybrid_refine_iterations"] = 10;

  d["stepsize_power_reference"] = false; // 仅参考，不用于步长
