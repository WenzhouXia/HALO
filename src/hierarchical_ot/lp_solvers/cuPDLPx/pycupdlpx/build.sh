...
Env overrides (optional):
  ...
  CUDA_HOME=/usr/local/cuda-12.3               # 指定 Toolkit 根（否则用当前 PATH 中的 nvcc）
USAGE
}
...
#   release  (默认)  Release 构建并安装到当前 Conda 环境
#   debug             Debug 构建并安装到当前 Conda 环境
#   clean             仅清理 build 目录
#   test              在 release 构建并安装后，运行 python test_dual_ot_loadData.py
# USAGE
# }
...
# ROOT_DIR="$(pwd)"                   # 假定在 cuPDLPx 根目录执行
# BUILD_DIR="$ROOT_DIR/build"
# PYTHON="$(command -v python)"
# SITE_PACKAGES="$($PYTHON - << 'PY'
# import sysconfig
...
# # 允许通过环境变量覆盖（如需指定 GPU 架构等）
# : "${EXTRA_CMAKE_FLAGS:=}"
# # 典型例子：
# #   EXTRA_CMAKE_FLAGS="-DCMAKE_CUDA_ARCHITECTURES=80;86"
...
# # -------- 安装 Python 扩展 --------
# # 约定 Python 扩展名为 pycupdlpx*.so
# EXT_SO="$(find "$BUILD_DIR" -type f -name 'pycupdlpx*.so*' | head -n1 || true)"
# if [[ -z "${EXT_SO}" ]]; then
#   echo "❌ 未找到 Python 扩展模块 (pycupdlpx*.so)。"
#   echo "   请确认 CMake 构建选项 BUILD_PYTHON=ON 且目标名为 pycupdlpx（或修改脚本中的匹配名）。"
#   exit 1
# fi
