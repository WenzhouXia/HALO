...
#     [9.0, 12.0, 13.0,  9.0],
# ], dtype=np.float64)
# c = C.reshape(-1, order='C')  # 变量顺序：(i,j) → i*nj + j

# # 构造 A 的 CSC：每列两个 1，分别在行 i 与行 (ni + j)
...
# ub  = np.full(n, np.inf, dtype=np.float64) # 无上界
# nEqs = m

# # ----- 用 cuPDLPx 求解 -----
# px.set_free_mode(int(os.getenv("CUPDLPX_FREE_MODE", "1")))  # 兼容：绑定里是 no-op
...
# params["eps_infeasible"]        = 1e-12   # 检测不（近）可行时用，适当更严一些
# # 关键：关闭缩放，避免回缩放不一致
