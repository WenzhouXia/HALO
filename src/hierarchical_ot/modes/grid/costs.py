...
def _grid_resolution_from_points(points: np.ndarray) -> int:
    ...
    # Hierarchical_OT stores grid coordinates normalized to [0, 1) at each level,
    # while MGPD kernels use integer lattice resolution. Infer the lattice size
    # from the number of unique coordinates per axis so both conventions map to
