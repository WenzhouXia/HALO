"""Runtime profiling helpers with minimal CUDA synchronization."""
...
class NoOpProfiler:
    """Disabled profiler: keeps overhead minimal and never synchronizes CUDA."""
...
class RuntimeProfiler:
    """Enabled profiler with phase-level structure and optional CUDA sync."""
    ...
    def _iter_component_buckets(self) -> Iterable[DefaultDict[str, float]]:
        ...
        if self._current_level is not None:
            yield self._current_level["components"]  # type: ignore[misc]
        if self._current_iteration is not None:
            yield self._current_iteration["components"]  # type: ignore[misc]
        if self._current_phase is not None:
            yield self._current_phase["components"]  # type: ignore[misc]
    ...
    def start_iteration(self, level_idx: int, inner_iter: int) -> None:
        ...
        level["iterations"].append(iteration)  # type: ignore[misc]
        self._current_level = level
    ...
    def start_phase(self, name: str, *, gpu_possible: bool = False) -> None:
        ...
        if self._current_iteration is not None:
            self._current_iteration["phases"][name] = phase  # type: ignore[index]
        elif self._current_level is not None:
            self._current_level["phases"][name] = phase  # type: ignore[index]
        else:
    ...
    def end_phase(self) -> None:
        ...
        if self._current_iteration is not None:
            if self._current_level is not None:
                self._current_level["phase_times"][phase_name] += dt  # type: ignore[index]
            self._current_iteration["phase_times"][phase_name] += dt  # type: ignore[index]
        elif self._current_level is not None:
            self._current_level["phase_times"][phase_name] += dt  # type: ignore[index]
        else:
    ...
    def _snapshot_phase_dict(self, phases: Dict[str, object]) -> Dict[str, object]:
        ...
        for name, phase in phases.items():
            phase_dict = phase  # type: ignore[assignment]
            ...
            out[str(name)] = {
                "time": dt,
                "components": self._finalize_component_map(phase_dict["components"]),  # type: ignore[arg-type]
            }
        return out
    ...
    def _snapshot_iteration(self, iteration: Dict[str, object]) -> Dict[str, object]:
        ...
        return {
            ...
            "time": iter_time,
            "components": self._finalize_component_map(iteration["components"]),  # type: ignore[arg-type]
            "phase_times": self._finalize_phase_map(iteration["phase_times"]),  # type: ignore[arg-type]
            "phases": self._snapshot_phase_dict(iteration["phases"]),  # type: ignore[arg-type]
        }

    def _snapshot_level(self, level: Dict[str, object]) -> Dict[str, object]:
        iterations = [
            self._snapshot_iteration(iteration)
            for iteration in level["iterations"]  # type: ignore[assignment]
        ]
        ...
        return {
            ...
            "components": self._finalize_component_map(level["components"]),  # type: ignore[arg-type]
            "phase_times": self._finalize_phase_map(level["phase_times"]),  # type: ignore[arg-type]
            "phases": self._snapshot_phase_dict(level["phases"]),  # type: ignore[arg-type]
