...
typedef struct
{
	...
	int use_dual_nnz_gate;
	double dual_nnz_factor;
	double dual_nnz_abs_tol;
} termination_criteria_t;

typedef enum
{
	STEP_SIZE_POWER_ITERATION = 0,
...
typedef struct
{
	...
	///
	int step_size_method;
	double step_size_safety;
	int power_max_iterations;
	double power_tolerance;
	int hybrid_refine_iterations;
